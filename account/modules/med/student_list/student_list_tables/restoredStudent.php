...
$table_log = db::arr("SELECT tl.USER_ID, tl.LOG_DATE, tl.ACTION, tl.COMMENT, sl.ID STUDENT_ID FROM `table_log` tl
    LEFT JOIN `student_list` sl ON tl.ITEM_ID = sl.ID
    WHERE tl.ACTION='restore_student' 
    AND tl.`LOG_DATE` BETWEEN '$from' AND '$to'
    AND sl.ORG_ID='$org_id'
